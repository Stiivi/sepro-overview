...
the simulation are _selector_ and _modifier_. The _selector_ specifies which
...
simulation controller but have no direct effect on the simulation.
...
effect on the simulation itself. The control actions are: `NOTIFY`, `TRAP` and
`HALT`.
...
payload. Notifications don’t interrupt the simulation and notification
handlers are not allowed to alter the state of the simulation.

`TRAP` interrupts the simulation and sends a message to the controller.
Similar to the notifications, traps are described by set of symbols. Traps are
used to hand control to the simulation controller.

The controller receives counts of traps that occurred and may hand the control
to the user or other systems. Simulation can be resumed after a trap
interruption and the trap handlers are allowed to alter the state of the
simulation.

`HALT` is a special control action that interrupts the simulation and
disallows it’s continuation. It can be thought as a `TRAP` with special meaning
and treatment. The action can be used to signal when an invalid simulation
state has been reached and it makes no sense to continue the simulation.
...
the simulation completely or clear the _halted_ state and allow the
simulation to be resumed.
